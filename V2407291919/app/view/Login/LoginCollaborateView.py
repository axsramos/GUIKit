...
class LoginCollaborateView(UIForm.UIForm):
    ...
    def create(self):
        ...
        frm_form.pack(side="left", pady=30)
        # /side left

        # side right
        frm_banner = Frame(frm_body, bg="#FFFFFF")

        frm_group = Frame(frm_banner, bg="#FFFFFF")
        Label(frm_group, image=self.data_image_banner, bg="#FFFFFF").pack()
        frm_group.pack(fill="x", padx=35, pady=2)

        frm_banner.pack(side="right", pady=30)
        # /side right

        frm_body.pack(fill="x")

        self.set_size_window(790, 450, 30)
